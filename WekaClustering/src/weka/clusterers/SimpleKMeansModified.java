/*
 * File: SimpleKMeansModified.java
 * Auth: Chen Haoting (chen0783@e.ntu.edu.sg)
 * Date: 2014-11-07
 * Desc: This program is used in the project of CZ4032 Data Minining of School of Computer Engineering in Nanyang Technological Univeristy (SG).
 * 		This program is modified from SimpleKMeans.java which is an original clustering algorthm of Weka. 
 * 		Additional methods added into the original code for searching the dominant class in the clusters (specially for Adult dataset)		
 * 
 * ------------------------------------------------------------------------------------
 */
...
public class SimpleKMeansModified extends RandomizableClusterer 
implements NumberOfClustersRequestable, WeightedInstancesHandler {
	...
	/** for serialization. */
	  ...
	  /** [chen0783] additional defined global variables */
	  protected Instances originalInstances;
	  protected int centroidIndexMapping;
	  protected Instances membersMapping;
	   ...
	   * [chen0783]  changed the default to be true
	   */
	  protected boolean m_dontReplaceMissing = true;
	   ...
	   * [chen0783] changed globalInfo 
	   * 
	   * @return a description of the evaluator suitable for displaying in the
	   *         explorer/experimenter gui
	   */
	  public String globalInfo() {
	    return "Modified K-Means for classification" ;
	  }
	  ...
	  public void buildClusterer(Instances data) throws Exception {
		...
		/** [chen0783] additional initial settings for removing the class label and save another copy for mapping */
	    Instances originalHeader = new Instances(data, 0);
	    Instances clusterData = new Instances(data);
	    clusterData.setClassIndex(-1);
	    clusterData.deleteAttributeAt(originalHeader.classIndex());
		originalInstances = new Instances(data);
	    
	    // can clusterer handle the data?
	    getCapabilities().testWithFail(clusterData);
	    ...
	    instances.deleteAttributeAt(data.numAttributes()-1);
	    ...
	    /** [chen0783] Add additional one slot for the int array for class count */ 
	    m_ClusterNominalCounts = new int[m_NumClusters][instances.numAttributes()+1][0];
	    ...
	    while (!converged) {
	      ...
	      // update centroids
	      //Logger.log(Logger.Level.INFO, "------ update centroids ------");
	      m_ClusterCentroids = new Instances(instances, m_NumClusters);
	    ...
	    /** [chen0783] call the mapping method for each cluster */
	    for (i = 0; i < m_NumClusters; i++) {
	    	centroidIndexMapping = i;
	    	membersMapping = tempI[i];
	    	mapping(centroidIndexMapping, membersMapping);
	    }
	  ...
	  protected double[] moveCentroid(int centroidIndex, Instances members,
	    boolean updateClusterInfo, boolean addToCentroidInstances) throws Exception {
	  ...
	  /**
	   * [chen0783]
	   * The method is for mapping the instance members of the cluster back to the original dataset whose class labels are not deleted
	   * It also updates the m_ClusterNominalCounts for the class label counts for each cluster
	   * 
	   * @param centroidIndex  index of the centroid in the cluster
	   * @param members			instance members of the cluster
	   * @throws Exception
	   */
	  private void mapping(int centroidIndex, Instances members) throws Exception
	  {
		//Logger.log(Logger.Level.INFO, "In mapping: centroid index:" +  centroidIndex);
	    Enumeration<Instance> memberInstances = members.enumerateInstances();

	    int classLabelNum = originalInstances.numAttributes()-1;
	    int[] classLabel = {0,0};
	    m_ClusterNominalCounts[centroidIndex][classLabelNum] = classLabel;
	    /*
	    Logger.log(Logger.Level.INFO, " -- m_ClusterNominalCounts[" + centroidIndex+"]["+ classLabelNum + "][0] = " 
	    		+ m_ClusterNominalCounts[centroidIndex][classLabelNum][0] 
	    		+ " \n --  m_ClusterNominalCounts[" + centroidIndex+"]["+ classLabelNum + "][1] = " 
	    		+ m_ClusterNominalCounts[centroidIndex][classLabelNum][1] );
	    */
  		for (Enumeration<Instance> e = memberInstances;;)
		{
  			if (e.hasMoreElements())
  			{
  				Instance inst = e.nextElement();
  				for (int k = 0; k<originalInstances.numInstances(); k++){
  		    		double[] originalInstanceArray = Arrays.copyOf(originalInstances.get(k).toDoubleArray(), originalInstances.get(k).toDoubleArray().length-1);
  		    		
  		    		boolean areEqual = Arrays.equals(inst.toDoubleArray(),originalInstanceArray);
			    		if (!areEqual)
			    			continue;
			    		else
			    		{
			    			//Logger.log(Logger.Level.INFO, "Two instances are equal");
				    		if (originalInstances.get(k).classValue() == 0.0)
			    				m_ClusterNominalCounts[centroidIndex][classLabelNum][0]++;
			    			else if (originalInstances.get(k).classValue() == 1.0)
			    				m_ClusterNominalCounts[centroidIndex][classLabelNum][1]++;
			    			else 
			    			{
			    				//Logger.log(Logger.Level.INFO, "Class Index is : " + originalInstances.get(k).classIndex());
			    				throw new Exception ("Class Label not acceptable");
			    			}
				    		break;
			    		}
  				}
	  ...
	  public void setDistanceFunction(DistanceFunction df) throws Exception {
	    if (!(df instanceof EuclideanDistance)
	      ...
	      throw new Exception(
	        "SimpleKMeansModified currently only supports the Euclidean and Manhattan distances.");
	    }
	  ...
	  /**
	   * Gets the current settings of SimpleKMeansModified.
	   * 
	  ...
	  public static void main(String[] args) {
	    runClusterer(new SimpleKMeansModified(), args);
	  }
	}
